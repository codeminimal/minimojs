...
public class XTemplates {
    ...
    public static String getTemplateName(String html, String defaultTemplateName,
        ...
        if (templateName != null && ((!webTemplate.containsKey(templateName) && !isIndexPage)
            ...
            InputStream is = X.getResource("/templates/" + templateName);
            if (is == null) {
    ...
    protected static String popupModalTemplate(String templateName) throws IOException {
        InputStream is = X.getResource("/templates/" + templateName);
        return preparePopupModalTemplate(is);
    }

    protected static String getFrameworkPopupModalTemplate() throws IOException {
        ...
        for (String tpl : modalTemplatesArray) {
            if (!tpl.trim().equals("")) {
                ...
                if (propertyName.equals(XDEFAULT_TEMPLATE_PROPERTY)) {
                    templateBody = XTemplates.getFrameworkPopupModalTemplate();
                } else {
                    templateBody = XTemplates.popupModalTemplate(tpl);
                }
